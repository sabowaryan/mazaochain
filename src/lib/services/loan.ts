...
import { ErrorHandler, MazaoChainError, ErrorCode, logger, retryUtils, ValidationSchemas } from '@/lib/errors'
import type { 
  LoanRequest, 
  LoanEligibility, 
  LoanDetails, 
...
export class LoanService {
   ...
   */
  async checkLoanEligibility(farmerId: string, requestedAmount: number): Promise<LoanEligibility> {
    const context = ErrorHandler.createContext({
      userId: farmerId,
      additionalData: { requestedAmount, operation: 'checkLoanEligibility' }
    });

    return retryUtils.forDatabase.execute(async () => {
      logger.info('Checking loan eligibility', { farmerId, requestedAmount });

      // Validate input parameters
      if (!farmerId) {
        throw new MazaoChainError(ErrorCode.VALIDATION_ERROR, 'Farmer ID is required', {
          userMessage: 'ID du fermier requis'
        });
      }

      if (requestedAmount <= 0) {
        throw new MazaoChainError(ErrorCode.VALIDATION_ERROR, 'Requested amount must be positive', {
          userMessage: 'Le montant demandé doit être positif'
        });
      }

      // Get farmer's portfolio to calculate available collateral
      const portfolio = await tokenizationService.getFarmerPortfolio(farmerId);
      ...
      if (!isEligible) {
        reasons.push(`Collatéral insuffisant. Requis: ${requiredCollateral} USDC, Disponible: ${availableCollateral} USDC`);
      }
      ...
      if (error) {
        throw new MazaoChainError(ErrorCode.DATABASE_ERROR, `Database error: ${error.message}`, {
          context,
      ...
      return result;
    }, 'checkLoanEligibility').catch(error => {
   ...
   */
  async createLoanRequest(request: LoanRequest): Promise<{ success: boolean; loanId?: string; error?: string }> {
    try {
      // Check eligibility first
      const eligibility = await this.checkLoanEligibility(request.borrowerId, request.requestedAmount)
      ...
      const { data: loan, error } = await this.supabase
        ...
        .insert({
          ...
          due_date: new Date(Date.now() + (request.repaymentPeriodMonths * 30 * 24 * 60 * 60 * 1000)).toISOString(),
          status: 'pending'
        })
        .select()
        .single()
      ...
      await this.supabase.rpc('send_notification', {
        recipient_id: request.borrowerId, // This should be the cooperative ID
  ...
  async getLoanById(loanId: string): Promise<LoanDetails | null> {
    try {
      ...
      return {
        ...
        lender: loan.lender ? {
          id: loan.lender.id,
          institution_name: (loan.lender as any)?.lender_profiles?.institution_name || 'Unknown'
        } : undefined,
        collateralTokens
      }
    } catch (error) {
   ...
   */
  async getUserLoans(userId: string, role: 'agriculteur' | 'cooperative' | 'preteur'): Promise<LoanDetails[]> {
    try {
      ...
      return loans?.map((loan: unknown) => ({
        ...
        borrower: loan.borrower ? {
          id: loan.borrower.id,
          nom: loan.borrower.farmer_profiles?.[0]?.nom || 'Unknown',
          email: 'email@example.com'
        } : undefined,
        lender: loan.lender ? {
          id: loan.lender.id,
          institution_name: loan.lender.lender_profiles?.[0]?.institution_name || 'Unknown'
        } : undefined
      })) || []
    } catch (error) {
   ...
   */
  async approveLoanRequest(approval: LoanApprovalRequest): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (approval.approved && approval.lenderId) {
        const disbursementResult = await this.automaticLoanDisbursement(approval.loanId, approval.lenderId)
        ...
        if (!disbursementResult.success) {
          ...
          console.error('Automatic disbursement failed:', disbursementResult.error)
        }
      }
   ...
   */
  async getLoanSummary(userId: string, role: 'agriculteur' | 'cooperative' | 'preteur'): Promise<LoanSummary> {
    try {
      const loans = await this.getUserLoans(userId, role)
      
      const totalLoans = loans.length
      const activeLoans = loans.filter(l => l.status === 'active').length
      const totalBorrowed = loans.reduce((sum, l) => sum + l.principal, 0)
      const totalRepaid = loans.filter(l => l.status === 'repaid').reduce((sum, l) => sum + l.principal, 0)
      const totalOutstanding = loans.filter(l => l.status === 'active').reduce((sum, l) => sum + l.principal, 0)
   ...
   */
  private calculateInterest(principal: number, annualRate: number, termMonths: number): InterestCalculation {
   ...
   */
  private async getCollateralTokens(loanId: string): Promise<CollateralToken[]> {
    try {
      // Get loan details
      ...
      const portfolio = await tokenizationService.getFarmerPortfolio(loan.borrower_id || '')
   ...
   */
  async automaticLoanDisbursement(loanId: string, lenderId: string): Promise<{ 
    success: boolean; 
    ...
    try {
      ...
      if (collateralTokens.length > 0) {
        ...
        await transactionReceiptService.recordTransaction(loan.borrower_id || '', {
          loanId,
      ...
      if (!disbursementResult.success) {
        ...
        throw new Error(`USDC disbursement failed: ${disbursementResult.error}`)
      }
      ...
      const receiptResult = await transactionReceiptService.generateDisbursementReceipt(receiptData)
   ...
   */
  async disburseLoan(disbursement: LoanDisbursement): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const result = await this.automaticLoanDisbursement(disbursement.loanId, loan.lender.id)
  ...
  async repayLoan(repayment: LoanRepayment): Promise<{ 
    ...
    try {
      ...
      if (repayment.paymentType === 'full') {
        const collateralTokens = await this.getCollateralTokensForLoan(repayment.loanId)
        ...
        if (loan.lender_id) {
          ...
          const distributionResult = await lenderService.distributeRepaymentToLender(
            repayment.loanId,
            repayment.amount
          )

          if (!distributionResult.success) {
            console.error('Failed to distribute repayment to lender:', distributionResult.error)
      ...
      console.log(`Loan repayment processed successfully for loan ${repayment.loanId}`)
   ...
   */
  async retryFailedDisbursement(loanId: string): Promise<{ success: boolean; error?: string }> {
    try {
      const loan = await this.getLoanById(loanId)
      if (!loan) {
        throw new Error('Loan not found')
      ...
      const result = await this.automaticLoanDisbursement(loanId, loan.lender.id)
   ...
   */
  async generateRepaymentSchedule(loanId: string): Promise<{ success: boolean; error?: string }> {
    try {
      const loan = await this.getLoanById(loanId)
      if (!loan) {
      ...
      if (error) {
        throw new Error(`Failed to generate repayment schedule: ${error.message}`)
      }

      return { success: true }
    } catch (error) {
      console.error('Error generating repayment schedule:', error)
      return { success: false, error: `Failed to generate repayment schedule: ${error}` }
    }
  }
  ...
  async getOutstandingBalance(loanId: string): Promise<{
    ...
    try {
      ...
      const outstandingPrincipal = loanData?.outstanding_balance || loan.principal
      
      // Calculate accrued interest (simplified - in real app would be more complex)
      const monthsElapsed = Math.max(1, Math.floor(
        (Date.now() - new Date(loan.created_at || '').getTime()) / (30 * 24 * 60 * 60 * 1000)
      ))
      const accruedInterest = outstandingPrincipal * loan.interest_rate * (monthsElapsed / 12)
   ...
   */
  async markInstallmentPaid(installmentId: string, paidAmount: number): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await this.supabase
        .from('repayment_schedule')
        .update({
      ...
      return { success: false, error: `Failed to update installment: ${error}` }
    }
  }
  ...
  async validateLoanForDisbursement(loanId: string): Promise<{
    ...
    try {
      ...
      const eligibility = await this.checkLoanEligibility(loan.borrower_id || '', loan.principal)
      if (!eligibility.isEligible) {
        errors.push(`Loan no longer eligible: ${eligibility.reasons?.join(', ')}`)
      }
