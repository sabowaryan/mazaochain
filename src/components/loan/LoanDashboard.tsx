...
import { TransactionErrorHandler, TransactionErrors } from './TransactionErrorHandler'
  ...
  const loadLoansData = useCallback(async () => {
    ...
    try {
      ...
      const [loansData, summaryData] = await Promise.all([
        loanService.getUserLoans(user.id, effectiveRole as 'agriculteur' | 'cooperative' | 'preteur'),
        loanService.getLoanSummary(user.id, effectiveRole as 'agriculteur' | 'cooperative' | 'preteur')
      ])
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{summary.totalLoans}</p>
        ...
        <CardHeader>
          <CardTitle>
            {profile?.role === 'agriculteur' ? 'Mes Prêts' : 
             profile?.role === 'cooperative' ? 'Prêts des Agriculteurs' : 
             'Prêts Financés'}
          </CardTitle>
          <CardDescription>
            {loans.length === 0 
                ...
                <div key={loan.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(loan.status || 'pending')}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{loan.collateral_amount.toFixed(2)} USDC</p>
                    </div>
                    <div>
                  ...
                  {profile?.role !== 'agriculteur' && loan.borrower && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-sm text-gray-600">
                        Emprunteur: <span className="font-medium">{loan.borrower.nom}</span>
                      </p>
                    </div>
                  )}
                  ...
                  {profile?.role === 'agriculteur' && loan.lender && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-sm text-gray-600">
                        Prêteur: <span className="font-medium">{loan.lender.institution_name}</span>
                      </p>
                    </div>
                  )}
